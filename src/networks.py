...
class Downsample(nn.Module):
    def __init__(self, channels, pad_type="reflect", filt_size=3, stride=2, pad_off=0):
        ...
        self.register_buffer(
            "mono", filt[None, None, :, :].repeat((self.channels, 1, 1, 1))
        )
...
def get_scheduler(optimizer, config):
    ...
    """
    if config.scheduler.lr_policy == "linear":

        def lambda_rule(epoch):
            lr_l = 1.0 - max(
                0, epoch + config.epoch_count - config.scheduler.n_epochs
            ) / float(config.scheduler.n_epochs_decay + 1)
            return lr_l

        scheduler = lr_scheduler.LambdaLR(optimizer, lr_lambda=lambda_rule)
    elif config.scheduler.lr_policy == "step":
        scheduler = lr_scheduler.StepLR(
            optimizer, step_size=config.scheduler.lr_decay_iters, gamma=0.1
        )
    elif config.scheduler.lr_policy == "plateau":
        scheduler = lr_scheduler.ReduceLROnPlateau(
            optimizer, mode="min", factor=0.2, threshold=0.01, patience=5
        )
    elif config.scheduler.lr_policy == "cosine":
        scheduler = lr_scheduler.CosineAnnealingLR(
            optimizer, T_max=config.scheduler.n_epochs, eta_min=0
        )
    else:
        return NotImplementedError(
            "learning rate policy [%s] is not implemented", config.scheduler.lr_policy
        )
    return scheduler
...
def define_G(config=None, gpu_ids=[]):
    """Create a generator
    ...
    input_nc = config.network.input_nc
    output_nc = config.network.output_nc
    ngf = config.network.ngf
    netG = config.network.netG
    norm = config.network.norm
    use_dropout = config.network.no_dropout
    init_type = config.network.init_type
    init_gain = config.network.init_gain
    no_antialias = config.network.no_antialias
    no_antialias_up = config.network.no_antialias_up
    padding_type = config.network.padding_type
    net = None
...
def define_F(config=None, gpu_ids=[]):
    netF = config.network.netF
    init_type = config.network.init_type
    init_gain = config.network.init_gain
    net = None
...
def define_D(config=None, gpu_ids=[]):
    ...
    """
    input_nc = config.network.output_nc
    ndf = config.network.ndf
    netD = config.network.netD
    n_layers_D = config.network.n_layers_D
    norm = config.network.norm
    init_type = config.network.init_type
    init_gain = config.network.init_gain
    no_antialias = config.network.no_antialias
    ...
    if netD == "basic":  # default PatchGAN classifier
        ...
        net = StyleGAN2Discriminator(
            input_nc, ndf, n_layers_D, no_antialias=no_antialias, config=config
        )
    else:
        raise NotImplementedError(
            ...
            if num_patches > 0:
                if patch_ids is not None:
                    ...
                    patch_id = torch.tensor(
                        patch_id[: int(min(num_patches, patch_id.shape[0]))],
                        dtype=torch.long,
                        device=feat.device,
                    )  # .to(patch_ids.device)
...
class ResnetGenerator(nn.Module):
    ...
    def __init__(
        ...
        if config.thermal.is_fpa_input:
            input_nc += 1
        ...
        for i in range(n_downsampling):  # add upsampling layers
            ...
            in_ch = (
                ngf * mult + 1 if config.thermal.is_fpa_input and i == 0 else ngf * mult
            )
            if no_antialias_up:
                model += [
                    nn.ConvTranspose2d(
        ...
        model += [
            nn.Conv2d(
                ngf, output_nc, kernel_size=7, padding=3, padding_mode=padding_type
            )
        ]
        model += [nn.Tanh()]

        self.upsample = nn.Sequential(*model)
        model.clear()
    ...
    @property
    def model(self):
        return [*self.downsample, *self.res_blocks, *self.upsample]

    def forward(self, input, layers=[], encode_only=False, src_fpa=None, tgt_fpa=None):
        if -1 in layers:
        ...
        if len(layers) > 0:
            ...
            feats = []
            for layer_id, layer in enumerate(self.model):
                if layer_id == 0:  # downsample
                    feat = self.append_fpa(feat, src_fpa)
                elif layer_id == 15:  # upsample
                    feat = self.append_fpa(feat, tgt_fpa)
                # print(layer_id, layer)
                feat = layer(feat)
            ...
            """Standard forward"""
            # append source fpa temperatures to the encoder part:
            in_with_fpa = self.append_fpa(input, src_fpa)
            x = self.downsample(in_with_fpa)

            x = self.res_blocks(x)

            # append target fpa temperatures to the decoder part:
            x_with_fpa = self.append_fpa(x, tgt_fpa)
            x = self.upsample(x_with_fpa)

            fake = x
            return fake

    @staticmethod
    def append_fpa(feature, fpa):
        if fpa is None:
            return feature
        feature_with_fpa = torch.empty(
            (feature.shape[0], feature.shape[1] + 1, *feature.shape[2:]),
            device=feature.device,
        )
        feature_with_fpa[:, :-1] = feature
        fpa_prot = torch.ones((*feature.shape[2:], len(fpa)), device=feature.device)
        feature_with_fpa[:, -1] = (fpa * fpa_prot).transpose(0, -1)
        return feature_with_fpa
...
class PixelWiseAffine(torch.nn.Module):
    def __init__(self, spat_dims=(256, 256)):
        super().__init__()
        self.weight = torch.nn.Parameter(torch.ones(spat_dims, requires_grad=True))
        self.bias = torch.nn.Parameter(torch.ones(spat_dims, requires_grad=True))
